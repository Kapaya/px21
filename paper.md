...
abstract: |
 ...
 In this paper, we present our progress towards extending Wildcard to empower end-users to create, extend and repair adapters by demonstration. We describe three design principles that guided our system's development and are applicable to other end-user webscraping and customization tools: (a) users should be able to scrape data and use it in a single environment, (b) users should be able to extend and repair the programs that scrape data via demonstration and (c) users should receive live feedback about their demonstrations. 
